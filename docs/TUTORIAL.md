...
LabEng 是一个实验室仪器控制软件。它的工作方式是把实验系统表示成一张框图：上位机、通信接口、仪器、测试例程各自是一个节点，节点之间的连线表示控制和数据流向。用户在设计页搭好框图并保存，软件生成一个 Python 例程文件；在运行页选中例程、连接仪器、填写参数后即可执行。
...
插件层        lab_engine/routines/  测试例程，每个 .py 文件一个
```

上层调用下层，下层不依赖上层。修改驱动层不会影响 GUI；新增例程只需要在 routines 目录下添加文件，不需要改动其他代码。

界面分三个 Tab：

- 运行：选择例程、连接仪器、填写参数、执行、查看实时曲线和日志。
- 例程结构：只读显示当前例程的框图。如果例程文件里保存了框图快照（SETUP_GRAPH），这里按保存时的布局还原显示。
- 测试系统设计：框图编辑器。新建项目向导、节点编辑、连线、面板配置、保存都在这个页面完成。
...
框图编辑器的保存结果直接写进例程 .py 文件：SETUP_GRAPH 变量保存框图快照，PANEL 变量保存面板配置。加载例程时引擎执行这个文件，把两个变量读出来。

早期版本框图存在单独的 .labsetup.json 文件里，界面上有"保存框图"和"生成例程代码"两个按钮。使用者经常搞混，改了框图忘记生成代码，或者反过来。合并成一个保存按钮之后这个问题消失了。分发一个例程文件就等于分发了代码、数据、布局、界面配置的全部内容。
...
按 README 的步骤跑一遍完整流程：新建项目向导、保存例程、选择 mock 仪器、运行。跑的时候留意三个问题：参数表单是谁生成的（routine_panel.py 读 PARAMS），例程列表是谁发现的（core/registry.py 的 discover），后台线程的例程为什么能实时更新界面（core/routine_context.py 的队列）。
...
core/registry.py，一百六十行左右。三个重点：InstrumentRegistry 是类级字典，register 的时候元数据入库；RoutineRegistry.discover() 递归扫描 routines 目录，用 importlib 动态加载每个 .py 文件，检查 NAME、PARAMS、run 三个属性，通过的进入注册表，失败的记入 last_report；RoutineMeta 是例程的档案，setup_graph 和 panel 字段保存内嵌的框图和面板。
...
core/setup_graph.py，四百行左右。纯数据类，与界面无关。Node 和 Edge 的序列化、validate_status() 的校验规则（比如例程节点的仪器端口没有连线就标红）都在这里。读懂它就知道 SETUP_GRAPH 里存的是什么。
...
练习一：复制 basic_iv_scan.py，改名 my_first.py，加一个 float 类型的参数 delay_s，在 run() 里 time.sleep(params["delay_s"])。刷新 GUI，确认表单多了一栏。做完这个练习就掌握了插件 API。
